					...
					converted -> tmpl.flatMap(tp -> tp.getCouchbaseClientFactory().getSession(null).flatMap(s -> {
						if (s == null || s.getAttemptContextReactive() == null) {
							...
							return s.getAttemptContextReactive()
									.insert(tp.doGetDatabase().block().bucket(tp.getBucketName()).reactive().scope(pArgs.getScope()!=null?pArgs.getScope():DEFAULT_SCOPE).collection(pArgs.getCollection()!=null?pArgs.getCollection():DEFAULT_COLLECTION),
